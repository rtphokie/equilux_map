...
def mapit(
    ...
    gdf_water.plot(
        ax=ax, color=BGCOLOR, edgecolor=BORDER_COLOR, linewidth=1.5, zorder=0
    )
    ...
    lon_grid, lat_grid, value_grid, land_mask = build_grid(
        us_precrs,
        min_lat,
        max_lat,
        min_lon,
        ...
        lon_refine_step=lon_refine_step,
    )
    ...
    mesh = ax.pcolormesh(
        x, y, value_grid, shading="nearest", cmap=cmap, norm=norm, zorder=20
    )
    # clip the color grid to the exact land geometry, coastline fidelity comes from the shapefile,
    # not from grid resolution.  This is safe because the accuracy of the line band transition is
    # maintained in the refinement step
    mesh.set_clip_path(polygon_to_path(us.geometry.union_all()), transform=ax.transData)

    # draw state and federal boundaries on top
    ...
    cbar.ax.set_xticklabels(
        [datetime.date.fromordinal(o).strftime("%b %-d") for o in ordinals]
    )
    cbar.set_label(
        f"date closest to 12 hours of daylight ({equinox.lower()} equinox, {year})",
        color=TEXT_COLOR,
    )
    ...
    fig.savefig(
        filename, dpi=dpi, facecolor=BGCOLOR, bbox_inches="tight", pad_inches=0.1
    )
...
def build_grid(
    ...
    This grid is refined adaptively in N-S and E-W directions, because the two
    ...
    # coarse pass - this is where essentially all the astronomy happens
    ...
    # coarse latitude interval that straddles its own transition and insert sufficient
    # finer samples there. This is an efficient way to achieve a smooth, more accurate line
    # without stair steps from undersampling.
    fine_lats = set()
    if refine_step and refine_step < step:
        for ref in unique_coarse_ref:
            ...
            for i in range(len(coarse_lats) - 1):
                if vals[i] != vals[i + 1]:
                    new_rows = np.arange(
                        coarse_lats[i], coarse_lats[i + 1], refine_step
                    )
        ...
        if refine_lon and lon_refine_step:
            for ref_a, ref_b in lon_pairs:
                if coarse_row_vals[ref_a] == coarse_row_vals[ref_b]:
                    continue
                sub_refs = np.arange(
                    ref_a, ref_b + lon_refine_step / 2, lon_refine_step
                )
    ...
    if fine_indices:
        pbar = tqdm(
            fine_indices,
            desc="computing equilux by latitude/longitude (refining transitions)",
        )
        for i in pbar:
            lat = lats[i]
...
def closest_equinox(today=None):
    ...
    year, name = min(
        candidates, key=lambda c: abs((find_equinox(*c) - today).total_seconds())
    )
    return year, name
...
if __name__ == "__main__":
    ...
    # get defaults from the function we'll be calling, apply those to the command line options
    mapit_defaults = {
        p.name: p.default for p in inspect.signature(mapit).parameters.values()
    }
    ...
    parser.add_argument(
        "--equinox",
        choices=["Vernal", "Autumnal"],
        default=default_equinox,
        help=f"which equinox to map (default: {default_equinox}, closest to today)",
    )
    parser.add_argument(
        "--step",
        type=float,
        default=mapit_defaults["step"],
    ...
    parser.add_argument(
        "--refine-step",
        type=float,
        default=mapit_defaults["refine_step"],
    ...
    parser.add_argument(
        "--lon-refine-step",
        type=float,
        default=mapit_defaults["lon_refine_step"],
    ...
    mapit(
        ...
        step=args.step,
        refine_step=args.refine_step or None,
        lon_refine_step=args.lon_refine_step or None,
    )
